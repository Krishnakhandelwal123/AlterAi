...
const buildEmbedCode = ({ shareUrl, slug, clone }) => ({
  iframe: `<iframe src="${shareUrl}?theme=dark&embed=true" width="100%" height="600px" frameborder="0" style="border-radius:16px;border:none;" allow="microphone" title="${clone.name} AI Clone"></iframe>`,
  script: `<script src="${getBaseUrlFromShareUrl(shareUrl)}/widget.js" data-slug="${slug}" data-theme="dark" data-position="bottom-right" data-color="${clone.avatar_color || '#00D4FF'}" async></script>`,
  widget: `<a href="${shareUrl}" target="_blank" style="background:${clone.avatar_color || '#00D4FF'};color:white;padding:12px 24px;border-radius:10px;text-decoration:none;font-family:sans-serif;">Chat with ${clone.name} AI</a>`
});
